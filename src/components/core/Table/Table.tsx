...
export const Table: React.FC<TableProps> = ({
  ...
  return (
    <div>
      <div
        data-testid="table"
        className={`w-full h-full mt-4 overflow-x-auto ${
      ...
      >
        <table className={`w-full ${tableContainer || "w-full h-full"}`}>
          {hasHeader ? (
            <thead>
              <tr className="border-b border-b-gray-300 py-3 px-4">
                {headers?.map(
                  (
                    header: {
                      name:
                    ...
                    <th
                      key={i}
                      className="
                              text-sm text-left font-normal text-gray-500 capitalize
                              px-4 py-3 whitespace-nowrap cursor-default"
                    >
                      {header.name}
                      {/* {header.sort && (
